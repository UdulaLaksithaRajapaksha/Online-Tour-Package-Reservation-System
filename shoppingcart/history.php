...
foreach ($db->query("SELECT * FROM " . config_item('cart', 'table_orders') . " o, " . config_item('cart', 'table_order_status_descriptions') . " osd WHERE o.user_id = '" . $session->get('user_id') . "' AND o.order_status_description_id = osd.order_status_description_id ORDER BY o.order_id DESC") as $row) {
	...
	foreach ($db->query("SELECT order_product_id, product_id, product_name, product_price, product_quantity, payment_company FROM " . config_item('cart', 'table_order_products') . " op, " . config_item('cart', 'table_orders') . " o WHERE o.user_id = '" . $session->get('user_id') . "' AND o.order_id = '" . $row['order_id'] . "' AND op.order_id = '" . $row['order_id'] . "'") as $product) {
