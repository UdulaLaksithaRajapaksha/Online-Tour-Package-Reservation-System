...
class Cart {
	...
	public function add_product($product_id, $options = array()) {
		...
		if ($this->db->row_count($sql)) {
			...
			/* if ($result['product_quantity'] < 6 && !$result['product_quantity'] == 0) {
									
				$this->error->set_error('The ' . $result['product_name'] . ' product is less than 5.');
			...
			if ($result['product_quantity'] == 0) {
				...
				$this->error->set_error('The ' . $result['product_name'] . ' product is not in stock.');
	...
	public function get_categories($category_id = 0) {
		...
		foreach ($this->db->query("SELECT * FROM " . self::$config['table_categories'] . " WHERE parent_id = '" . $category_id . "' AND category_status = 1 ORDER BY category_id ASC") as $row) {
